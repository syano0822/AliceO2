...
public:
  void run(o2::framework::ProcessingContext& pc) final
  {
    ...
    for(int iRUconf=0; iRUconf<nRUConf; ++iRUconf){
      ...
      conf.setName(getNameFromDecAddRUconf[arrAddRUConf[iRUconf]]);      
      conf.setType(0);
      ccdbConfig.emplace_back(conf);
    }

    for(int iALPIDEconf=0; iALPIDEconf<nALPIDEConf; ++iALPIDEconf){
      LOG(INFO)<<"("<<arrAddALPIDEConf[iALPIDEconf]<<")   "<<getNameFromDecAddALPIDEconf[arrAddALPIDEConf[iALPIDEconf]]<<" : "<<arrValALPIDEConf[iALPIDEconf];
      o2::mft::DCSConfigInfo conf;
      conf.clear();
      conf.setData(arrValALPIDEConf[iALPIDEconf]);
      conf.setAddDeci(arrValALPIDEConf[iALPIDEconf]);
      conf.setName(getNameFromDecAddALPIDEconf[arrAddALPIDEConf[iALPIDEconf]]);      
      conf.setType(1);
      ccdbConfig.emplace_back(conf);
    }
...
private:
  ...
  void sendConfigoutput(DataAllocator& output, T& payload, o2::ccdb::CcdbObjectInfo& info){
    ...
    output.snapshot(Output{o2::calibration::Utils::gDataOriginCDBWrapper, "DCS_CONFIG_FILE", 0}, info);
  }
...
DataProcessorSpec getDCSConsumerSpec(DetID det)
{
  ...
  return DataProcessorSpec{
    ...
    Inputs{{"confFile", ConcreteDataTypeMatcher{det.getDataOrigin(), "DCS_CONFIG_FILE"}, Lifetime::Timeframe},
      ...
      Outputs{},
    AlgorithmSpec{adaptFromTask<o2::mft::DCSConfigConsumer>()},
    Options{}};
}
