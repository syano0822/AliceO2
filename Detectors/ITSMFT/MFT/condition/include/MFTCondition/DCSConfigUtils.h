...
class DCSConfigUtils
{
 ...
 public:
  ...
  auto getAddress(std::string name, std::string type)
  {
  ...
  {
    return static_cast<std::string>((mAddressDictType[type])[add]);
  }
 ...
 private:
  ...
  void initDictionary()
  {
    ...
    std::vector<std::pair<std::string, uint>> pairDeadMap;
    int idDeadMap = 0;
    ...
    for (int iDeadMap = 0; iDeadMap < pairDeadMap.size(); ++iDeadMap) {
      std::pair<std::string, int> p = pairDeadMap[iDeadMap];
