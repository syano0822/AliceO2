...
class DCSUtils
{
 ...
 public:
  void init(DCSConfigInfo info)
  {
    info.getData(mData);
    info.getAdd(mAdd);
    info.getType(mType);
    info.getVersion(mVersion);
    connectNameAdd();
  }
  ...
  void getName(std::string& name){
    if(mType == 0) name = mMapAddNameRU[mAdd];
    else if(mType == 1) name = mMapAddNameALPIDE[mAdd];
  }
  
  void getTypeStr(std::string& name){
    if(mType==0) name = "RU";
    else if(mType==1) name = "ALPIDE";
  }
